...
export function normalizeToken(raw: Partial<DexToken> & { mint: string }): DexToken {
  return {
    mint: raw.mint,
    name: raw.name?.trim() || "Unknown",
    symbol: raw.symbol?.trim() || "???",
    imageUrl: raw.imageUrl || (raw as { image?: string }).image || "",
    creator: raw.creator || "",
    createdAt: raw.createdAt || Date.now(),
  };
}

export function filterTokens(tokens: DexToken[], query: string): DexToken[] {
...
export function shortMint(mint: string, start = 6, end = 4): string {
