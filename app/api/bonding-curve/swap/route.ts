...
export async function POST(req: NextRequest) {
  try {
    ...
    // user tanımlandıktan SONRA umi oluşturuluyor — payer kullanıcı olacak
    ...
    // —— PLATFORM FEE ATA —————————————————————————————————————————
    // ATA yoksa swap tx'ine instruction olarak ekliyoruz.
    // Payer kullanıcı olduğu için platform wallet bakiyesi gerekmez.
    // İlk swap'ta kullanıcı ~0.002 SOL rent öder, sonraki swap'larda ATA zaten var.
    ...
    if (PLATFORM_FEE_WALLET) {
      ...
      console.warn("PLATFORM_FEE_WALLET not set — fee will not be collected");
    }
