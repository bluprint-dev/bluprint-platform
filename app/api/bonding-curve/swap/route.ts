  ...
  try {
    ...
    if (feeWalletStr) {
      ...
      if (!feeAtaAccount.exists) {
        console.log("FEE_ATA not found, building separate tx:", feeAta.toString());
        const feeAtaBuilderTx = createAssociatedToken(umi, {
          mint: wsol,
          owner: feeWallet,
          payer: user,
        });
    ...
    const tx = await (await swapBuilder.setLatestBlockhash(umi)).buildAndSign(umi);
