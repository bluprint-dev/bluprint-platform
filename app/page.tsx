...
const F = {
  display: "'Outfit', 'Plus Jakarta Sans', 'DM Sans', system-ui, -apple-system, sans-serif",
...
function LiveActivityToast() {
  ...
  useEffect(() => {
    const push = () => {
      setItems(prev => [randomActivity(), ...prev].slice(0, 4));
    };
    push();
  ...
  return (
    <div style={{ position: "fixed", bottom: 24, left: 24, zIndex: 999, display: "flex", flexDirection: "column", gap: 8, pointerEvents: "none" }}>
      <AnimatePresence>
        {items.map((item) => (
          ...
          >
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: item.action === "just launched" ? "#9945FF" : "#14F195", boxShadow: `0 0 8px ${item.action === "just launched" ? "#9945FF" : "#14F195"}`, flexShrink: 0 }} />
...
function SlotsCounter() {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setSlots(s => Math.max(1, s - (Math.random() > 0.7 ? 1 : 0)));
    }, 8000);
    return () => clearInterval(iv);
  }, []);
  return (
    <div style={{
      ...
      <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#ff5050", boxShadow: "0 0 8px #ff5050", display: "inline-block", animation: "ping2 1.2s ease-out infinite" }} />
      <span style={{ fontFamily: F.mono, fontSize: 11, fontWeight: 700, color: "rgba(255,120,120,0.9)", letterSpacing: "0.1em" }}>
        ONLY <span style={{ color: "#ff5050", fontSize: 14 }}>{slots}</span> EARLY ACCESS SLOTS LEFT AT 0.05 SOL
      </span>
    </div>
  );
}
...
function ViewersBadge({ count }: { count: number }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 999, background: "rgba(153,69,255,0.07)", border: "1px solid rgba(153,69,255,0.2)" }}>
      <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#9945FF", boxShadow: "0 0 6px #9945FF", display: "inline-block", animation: "ping2 1.8s ease-out infinite" }} />
      <span style={{ fontFamily: F.mono, fontSize: 10, fontWeight: 700, color: "rgba(153,69,255,0.8)", letterSpacing: "0.1em" }}>
        {count} WATCHING NOW
      </span>
    </div>
  );
...
function LaunchTicker({ tokens }: { tokens: DexToken[] }) {
  ...
  const items = tokens.length > 0
    ? [...tokens, ...tokens]
    : Array.from({ length: 20 }, (_, i) => ({ name: `TOKEN${i}`, symbol: `TKN${i}`, mint: `${i}`, imageUrl: "", creator: "", createdAt: 0 }));

  return (
    <div style={{ overflow: "hidden", position: "relative", borderTop: "1px solid rgba(153,69,255,0.12)", borderBottom: "1px solid rgba(153,69,255,0.12)", background: "rgba(153,69,255,0.03)", padding: "10px 0" }}>
      ...
      <div ref={trackRef} style={{ display: "inline-flex", alignItems: "center", gap: 0, whiteSpace: "nowrap", willChange: "transform" }}>
        {items.map((t, i) => (
          <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "0 28px" }}>
            <span style={{ width: 5, height: 5, borderRadius: "50%", background: i % 3 === 0 ? "#14F195" : i % 3 === 1 ? "#9945FF" : "#fff", display: "inline-block", opacity: 0.6 }} />
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    link.href = "https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700;800;900&family=Space+Mono:wght@400;700&display=swap";
    link.rel = "stylesheet";
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes urgentFlash { 0%,100%{opacity:1;box-shadow:0 0 36px rgba(20,241,149,0.4),0 4px 20px rgba(20,241,149,0.2)} 50%{opacity:0.85;box-shadow:0 0 56px rgba(20,241,149,0.7),0 4px 32px rgba(20,241,149,0.4)} }
      ...
      <section style={{ position: "relative", zIndex: 10, minHeight: "100vh", display: "flex", alignItems: "center" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "96px 24px", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          ...
          <motion.div initial={{ opacity: 0, x: -32 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.75, ease: [0.22,1,0.36,1] }}
            style={{ display: "flex", flexDirection: "column", gap: 24 }}>

            {/* live badge */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "8px 16px", borderRadius: 999, background: "rgba(20,241,149,0.06)", border: "1px solid rgba(20,241,149,0.18)" }}>
            ...
            {/* social proof bar */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
              {/* avatar stack */}
      ...
      <section style={{ position: "relative", zIndex: 10, paddingTop: 80, paddingBottom: 80 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: "flex", alignItems: "center", justifyContent: "center", paddingTop: 96, paddingBottom: 96 }}>
              ...
              <motion.div key="empty" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", paddingTop: 96, paddingBottom: 96, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 24, marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontFamily: F.mono, fontSize: 10, color: "rgba(255,255,255,0.2)", letterSpacing: "0.06em" }}>Solana's fastest bonding-curve launchpad</p>
              </div>
            </div>
            <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
              {[["X / Twitter", "#"], ["Telegram", "#"], ["Docs", "#"], ["Solscan", "#"]].map(([label, href]) => (
                <a key={String(label)} href={String(href)} style={{ fontFamily: F.mono, color: "rgba(255,255,255,0.28)", textDecoration: "none", fontSize: 11, fontWeight: 600, letterSpacing: "0.05em" }}
